...
export function isIncompleteMouseSequence(buffer: string): boolean {
  ...
  // It could be a mouse sequence, but it's not complete yet.
  // We need to be careful not to wait forever on garbage that looks like a prefix.
  ...
  if (buffer.startsWith(SGR_EVENT_PREFIX)) {
    ...
    // Add a reasonable max length check to avoid hanging on garbage.
